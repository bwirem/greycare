...
export default function Edit({ order, fromstore, auth, priceCategories, facilityOptions }) {
    ...
    const handleProceed = (destination) => {
        ...
        if (destination === 'save') router.post(route('pharmacy0.billing.confirmUpdate', { order: order.id }), finalPayload);
        else if (destination === 'pay') router.post(route('pharmacy0.billing.confirmExistingPayment', { order: order.id }), finalPayload);
    };
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                        <form className="space-y-6" onSubmit={e => e.preventDefault()}>
                            ...
                            <div className="flex justify-end space-x-3 border-t pt-4">
                                <button onClick={() => { sessionStorage.removeItem(STORAGE_KEY); router.visit(route('pharmacy0.billing.index')); }} className="px-4 py-2 bg-gray-200 rounded-md">Close</button>
