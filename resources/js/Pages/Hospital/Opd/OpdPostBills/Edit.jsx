...
export default function Edit({ order, fromstore, auth, priceCategories, facilityOptions, billinggroups, billingsubgroups }) {
    ...
    const { data, setData, errors, processing } = useForm({
        ...
        billinggroup_id: order.billinggroup_id || null,
        billingsubgroup_id: order.billingsubgroup_id || null,
        billinggroupmembershipno: order.billinggroupmembershipno || null,
    ...
    const handleProceed = (destination) => {
        ...
        if (destination === 'save') router.post(route('outpatient4.confirmUpdate', { order: order.id }), finalPayload);
        else if (destination === 'pay') router.post(route('outpatient4.confirmExistingPayment', { order: order.id }), finalPayload);
    };
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                        <form className="space-y-6" onSubmit={e => e.preventDefault()}>
                            ...
                            <div className="flex justify-end space-x-3 border-t pt-4">
                                <button onClick={() => { sessionStorage.removeItem(STORAGE_KEY); router.visit(route('outpatient4.index')); }} className="px-4 py-2 bg-gray-200 rounded-md">Close</button>
