...
import { faSyringe, faHistory, faFileMedicalAlt, faHeartbeat } from '@fortawesome/free-solid-svg-icons';
...
export default function MedicationCreate({ 
    ...
    // --- State ---
    const [activeTab, setActiveTab] = useState('medication'); // 'medication', 'vitals', 'clinical_history'
    const [selectedRx, setSelectedRx] = useState(null);
    const [showModal, setShowModal] = useState(false);

    // --- Form State (For Medication Administration) ---
    ...
    // --- Modal Logic (Auto-Calculate Quantity) ---
    const openAdminModal = (rx) => {
        setSelectedRx(rx);

        // 1. Get raw input dosage
        const inputDosage = parseFloat(rx.dosage) || 0;
        let finalQty = 0;

        // 2. Calculation Logic: (Dose / Strength)
        if (inputDosage > 0) {
            ...
            if (details) {
                ...
                if (type === 0 && strength > 0 && inputDosage >= strength) {
                    // Solid (Tablets): Dose / Strength
                    finalQty = (inputDosage / strength); 
                } else if (type === 1 && bottleSize > 0) {
                    // Liquid: Usually Dose = ml, or specific calc based on concentration
                    // If you have concentration (mg/ml), logic changes here. 
                    // For now, assuming dosage is already in units to administer or ml.
                ...
                // No details, assume raw number (e.g. "2" tabs)
                finalQty = inputDosage; 
            }
        }

        // 3. Fallback to 1 if calc fails or results in 0
        const defaultQty = finalQty > 0 ? Math.ceil(finalQty * 100) / 100 : 1;
    ...
    const submit = (e) => {
        ...
        post(route('nursing1.store'), {
            onSuccess: () => {
                setShowModal(false);
                reset('prescription_id', 'quantity', 'remarks'); // Keep source_id/type
            }
        });
    };

    return (
        <HospitalLayout header={<h2>Nursing Station: {patient.first_name} {patient.last_name}</h2>}>
            <Head title="Nursing Care" />
            ...
            <div className="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-1/4 bg-white p-5 rounded-lg shadow-sm border border-gray-200 h-fit">
                    <div className="text-center mb-4">
                        <div className="h-16 w-16 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-3 text-2xl font-bold text-indigo-700 border-2 border-white shadow">
                    ...
                    <div className="border-t pt-3 text-sm text-gray-600 space-y-2">
                        ...
                        <div className="flex justify-between"><span>Type:</span> <span className="font-bold text-blue-600">{source_type}</span></div>
                    </div>
                </div>
                ...
                <div className="w-full md:w-3/4 bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col min-h-[600px]">
                    ...
                    <div className="flex border-b border-gray-200 bg-gray-50 rounded-t-lg overflow-x-auto">
                        ...
                        >
                            <FontAwesomeIcon icon={faHeartbeat} /> Vitals Monitor
                        </button>
