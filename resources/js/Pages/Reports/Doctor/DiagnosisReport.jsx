...
export default function DiagnosisReport({ auth, data, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        get(route('reports.doctor.diagnosis'));
    };
    ...
    const calculateTotals = () => {
        ...
        data.forEach(row => {
            Object.keys(totals).forEach(key => {
                totals[key].m += parseInt(row.stats[key].m || 0);
                totals[key].f += parseInt(row.stats[key].f || 0);
                totals[key].t += parseInt(row.stats[key].t || 0);
            });
        });
        return totals;
    };
    ...
    const getTitle = () => {
        if(searchData.report_type === 'opd') return "OPD Morbidity Report (By Age & Gender)";
        if(searchData.report_type === 'ipd') return "IPD Morbidity Report (By Age & Gender)";
        return "Global ICD-10 Report (By Age & Gender)";
    };
                ...
                <div className="mx-auto w-full px-2 sm:px-4 lg:px-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6 border border-gray-100 dark:border-gray-700">
                        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1 uppercase">Department</label>
                                <select
                                    className="block w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600"
                                    value={searchData.report_type}
                                    onChange={e => setData('report_type', e.target.value)}
                                >
                                    <option value="icd">All Contexts</option>
                            ...
                            <div>
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md shadow transition font-medium text-sm"
                                >
                    ...
                    <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-300 dark:border-gray-600 overflow-x-auto">
                        ...
                        <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-600 border-collapse">
                            <thead className="bg-gray-100 dark:bg-gray-700">
                                ...
                                {/* Row 2: Gender Headers */}
                                <tr>
                                    {/* Repeater for each group */}
                                    {[1,2,3,4,5,6].map((i) => (
                                ...
                                {data.length > 0 ? (
                                    data.map((row, index) => (
                                        <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <td className="px-2 py-1 border-r border-gray-200 dark:border-gray-600 text-center text-xs text-gray-500">{index + 1}</td>
                                            <td className="px-2 py-1 border-r border-gray-200 dark:border-gray-600 text-xs font-medium text-gray-800 dark:text-gray-200">
                                                {row.name}
                                                {row.mapped && <span className="block text-[10px] text-gray-400 font-mono">({row.code})</span>}
                                            </td>

                                            {/* U 1M */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats.u1m.m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats.u1m.f || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-900">{row.stats.u1m.t || '-'}</td>

                                            {/* 1M - 1Y */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['1m1y'].m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['1m1y'].f || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-900">{row.stats['1m1y'].t || '-'}</td>

                                            {/* 1Y - 5Y */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['1y5y'].m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['1y5y'].f || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-900">{row.stats['1y5y'].t || '-'}</td>

                                            {/* 5Y - 60Y */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['5y60'].m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats['5y60'].f || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-900">{row.stats['5y60'].t || '-'}</td>

                                            {/* > 60Y */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats.o60y.m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700">{row.stats.o60y.f || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-900">{row.stats.o60y.t || '-'}</td>

                                            {/* Grand */}
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">{row.stats.grand.m || '-'}</td>
                                            <td className="px-1 text-center text-xs border-r border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">{row.stats.grand.f || '-'}</td>
                                            <td className="px-1 text-center text-xs font-bold text-indigo-600 bg-gray-100 dark:bg-gray-800">{row.stats.grand.t}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
