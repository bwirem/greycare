...
export default function RadAnalysisReport({ auth, reportData, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        get(route('reports.rad.analysis'), { preserveState: true });
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Radiology Exam Analysis</h2>}>
            <Head title="Rad Analysis" />
            <div className="py-12 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-8 print:shadow-none print:p-0">
                    <div className="text-center mb-8">
                        <h1 className="text-2xl font-bold uppercase text-gray-800 dark:text-white">Exam Volume Analysis</h1>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-purple-50 dark:bg-purple-900/30">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-purple-800 dark:text-purple-200 uppercase tracking-wider">Procedure Name</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-purple-800 dark:text-purple-200 uppercase tracking-wider">Modality</th>
                                    <th className="px-6 py-3 text-center text-xs font-bold text-purple-800 dark:text-purple-200 uppercase tracking-wider">Total Requests</th>
                                    <th className="px-6 py-3 text-center text-xs font-bold text-purple-800 dark:text-purple-200 uppercase tracking-wider">Reported</th>
                                    <th className="px-6 py-3 text-right text-xs font-bold text-purple-800 dark:text-purple-200 uppercase tracking-wider">Completion Rate</th>
                                </tr>
                            </thead>
                                ...
                                {reportData.volumes.map((row, i) => {
                                    ...
                                    return (
                                        <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{row.modality || '-'}</td>
