...
export default function RadRequestsReport({ auth, reportData, procedures, filters }) {
    const { data, setData, get, processing } = useForm({
        start_date: filters.start_date || '',
        end_date: filters.end_date || '',
        status: filters.status || '',
        procedure_id: filters.procedure_id || '',
    });

    const handleSearch = (e) => {
        e.preventDefault();
        get(route('reports.rad.requests'), { preserveState: true });
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Radiology Activity Log</h2>}>
            <Head title="Radiology Log" />
            <div className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow mb-6 print:hidden">
                    <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                        ...
                        <select value={data.status} onChange={e => setData('status', e.target.value)} className="rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600">
                            ...
                            <option value="Image Taken">Image Taken</option>
                            <option value="Reported">Reported</option>
                            <option value="Rejected">Rejected</option>
                        </select>

                        <select value={data.procedure_id} onChange={e => setData('procedure_id', e.target.value)} className="rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600">
                ...
                <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-8 print:shadow-none print:p-0">
                    <div className="text-center mb-6">
                        <h1 className="text-2xl font-bold uppercase text-gray-800 dark:text-white">Radiology Activity Log</h1>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th className="px-4 py-2 text-left font-bold text-gray-700 dark:text-gray-300">Accession #</th>
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {reportData.rows.data.map(row => (
                                    <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <td className="px-4 py-2 font-mono text-xs text-gray-500 dark:text-gray-400">{row.accession}</td>
