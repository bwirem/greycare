...
export default function RadReportsDashboard({ auth, stats }) {
    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Radiology Reporting Hub</h2>}>
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                        ...
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border-l-4 border-orange-500">
                            <span className="text-gray-500 text-xs uppercase font-bold">To Be Imaged</span>
                            <span className="block text-2xl font-bold text-gray-800 dark:text-white">{stats.pending_imaging}</span>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border-l-4 border-purple-500">
                            <span className="text-gray-500 text-xs uppercase font-bold">To Be Reported</span>
                            <span className="block text-2xl font-bold text-gray-800 dark:text-white">{stats.pending_reporting}</span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        <ReportCard
                            title="Radiology Request Log"
                            description="Detailed list of X-rays, Ultrasounds, and CT scans ordered."
                            icon={faListAlt}
                            iconBgColor="bg-blue-600"
                            linkHref={route('reports.rad.requests')}
                            linkText="View Activity"
                        />
                        <ReportCard
                            title="Exam Volume Analysis"
                            description="Most frequent procedures and completion stats."
                            icon={faChartPie}
                            iconBgColor="bg-purple-600"
                            linkHref={route('reports.rad.analysis')}
                            linkText="View Statistics"
                        />
                        <ReportCard
                            title="Patient History"
                            description="Search specific patient imaging history."
                            icon={faXRay}
