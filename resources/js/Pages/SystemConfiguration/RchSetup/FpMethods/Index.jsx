...
export default function FpMethodIndex({ auth, methods, success }) {
    ...
    const handleConfirmDelete = () => {
        router.delete(route("systemconfiguration14.fpmethods.destroy", modalState.idToDelete), {
            onSuccess: () => setModalState({ isOpen: false, idToDelete: null }),
        });
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Family Planning Methods</h2>}>
            <Head title="FP Methods" />
            <div className="py-12">
                <div className="mx-auto max-w-5xl sm:px-6 lg:px-8">
                    <div className="bg-white shadow-sm sm:rounded-lg p-6">
                        <div className="mb-6 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-gray-700">Methods List</h3>
                            <div className="flex gap-2">
                                <Link href={route("systemconfiguration14.fpmethods.create")} className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 flex items-center">
                                    ...
                                    {methods.data.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-center space-x-4">
                                                <Link href={route("systemconfiguration14.fpmethods.edit", item.id)} className="text-blue-600 hover:text-blue-900">
                        ...
                        <Pagination class="mt-6" links={methods.links} />
                    </div>
                </div>
            </div>
            <Modal isOpen={modalState.isOpen} onClose={() => setModalState({ isOpen: false, idToDelete: null })} onConfirm={handleConfirmDelete} title="Delete Method" message="Are you sure?" />
        </AuthenticatedLayout>
    );
}