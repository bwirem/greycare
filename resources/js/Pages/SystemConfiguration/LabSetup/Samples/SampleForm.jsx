...
export default function SampleForm({ sample = null }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (sample) {
            put(route('systemconfiguration6.samples.update', sample.id));
        } else {
            post(route('systemconfiguration6.samples.store'));
        }
    };
    ...
    return (
        <form onSubmit={submit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4 border-t pt-4">
                <Link href={route('systemconfiguration6.samples.index')} className="text-gray-600 px-4 py-2">Cancel</Link>
