...
export default function IcdImport({ auth, flash }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('systemconfiguration5.diagnoses.import.store'));
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Import ICD-10</h2>}>
            <Head title="Import ICD" />
            ...
            <div className="py-12 max-w-4xl mx-auto sm:px-6 lg:px-8">
                <div className="bg-white p-6 shadow-sm sm:rounded-lg">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <a href={route('systemconfiguration5.diagnoses.index')} className="text-gray-500 hover:text-gray-700">
                            <FontAwesomeIcon icon={faArrowLeft} className="mr-1"/> Back
                        </a>
                        <a href={route('systemconfiguration5.diagnoses.template')} className="text-blue-600 hover:text-blue-800 font-medium text-sm flex items-center">
                    ...
                    <form onSubmit={submit} className="space-y-6 border-2 border-dashed border-gray-300 rounded-lg p-10 text-center hover:bg-gray-50 transition-colors">
                        ...
                        {progress && (
                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                                <div className="bg-cyan-600 h-2.5 rounded-full" style={{ width: `${progress.percentage}%` }}></div>
                            </div>
                        )}
                        ...
                        <button 
                            ...
                            className="w-full bg-cyan-600 text-white px-6 py-3 rounded-md font-semibold hover:bg-cyan-700 disabled:opacity-50 flex justify-center items-center gap-2"
                        >
                    ...
                    <div className="mt-6 text-sm text-gray-500">
                        ...
                        <ul className="list-disc list-inside mt-2">
                            <li><strong>code</strong>: ICD-10 Code (Required, Unique).</li>
                            <li><strong>name</strong>: Disease Name (Required).</li>
                            <li><strong>group</strong>: Category Name (Optional, e.g., 'Infectious Diseases').</li>
                        </ul>
                    </div>
                </div>
            </div>
