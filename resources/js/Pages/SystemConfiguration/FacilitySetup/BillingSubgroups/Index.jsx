...
export default function SubgroupIndex({ auth, subgroups, success, filters }) {
    ...
    const handleSearch = (e) => {
        ...
        router.get(route("systemconfiguration5.billingsubgroups.index"), 
    ...
    const handleConfirmDelete = () => {
        router.delete(route("systemconfiguration5.billingsubgroups.destroy", modalState.idToDelete), {
            onSuccess: () => setModalState({ isOpen: false, idToDelete: null }),
        });
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold">Billing Subgroups (Schemes)</h2>}>
            <Head title="Billing Subgroups" />
            <div className="py-12">
                    ...
                    <div className="bg-white shadow-sm sm:rounded-lg p-6">
                        ...
                        <div className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div className="relative w-full md:w-64">
                                ...
                                <input type="text" placeholder="Search..." value={searchData.search} onChange={handleSearch} className="w-full rounded-md border-gray-300 pl-10" />
                            </div>
                            <div className="flex gap-2">
                                <Link href={route("systemconfiguration5.billingsubgroups.create")} className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 flex items-center shadow">
                                    ...
                                    {subgroups.data.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-center space-x-4">
                                                <Link href={route("systemconfiguration5.billingsubgroups.edit", item.id)} className="text-blue-600 hover:text-blue-900">
                        ...
                        <Pagination class="mt-6" links={subgroups.links} />
                    </div>
                </div>
            </div>
            <Modal isOpen={modalState.isOpen} onClose={() => setModalState({ isOpen: false, idToDelete: null })} onConfirm={handleConfirmDelete} title="Delete Scheme" message="Are you sure?" />
        </AuthenticatedLayout>
    );
}