...
use App\Models\Inventory\SIV_Supplier;

use App\Models\Facility\FacilityOption;
use App\Models\Inventory\IVProductCostLog;

use App\Models\Accounting\ACCMakePayment; // Import MakePayment model
use App\Models\Accounting\ChartOfAccount; // Import Chart of Account model
use App\Models\Accounting\ChartOfAccountMapping; // Import Chart of Account Mapping model
        ...
        try {
            DB::transaction(function () use ($purchase, $validated, $request) {
                ...
                foreach ($validated['items_received'] as $receivedItemData) {
                    ...
                    IVProductCostLog::create(['sysdate' => now(), 'transdate' => $purchase->transdate, 'product_id' => $product->id, 'costprice' => $newCostPrice]);
